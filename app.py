...
# Page configuration
st.set_page_config(
    page_title="Professional Beam Analysis (2D)",
    page_icon="🏗️",
    layout="wide",
)
...
st.caption(
    "Beam diagrams with support configurations, reaction forces visualization,"
    " moving load simulation, and safety checks"
)
...
with st.sidebar:
  ...
  len_unit = st.radio(
      "Length Unit for Beam", ["Inches (in)", "Feet (ft)"], horizontal=True
  )
  if "Feet" in len_unit:
    L_ft = st.number_input(
        "Beam Length L (ft)", min_value=0.1, value=16.0, step=1.0
    )
    L = L_ft * 12.0
  else:
    L = st.number_input(
        "Beam Length L (in)", min_value=1.0, value=192.0, step=12.0
    )

  st.subheader("Force Unit Selection")
  force_unit = st.selectbox(
      "Select Force Unit", ["kips (1 kip = 1,000 lbs)", "Pounds (lbs)"]
  )
  ...
  n_loads = st.number_input(
      "Number of Point Loads", min_value=0, max_value=5, value=1
  )
  ...
  if enable_walker:
    ...
    if "Pounds" in force_unit:
      walker_wt_lb = st.number_input(
          "Moving Load Weight (lbs)",
          min_value=10.0,
          value=default_wt_lb,
          step=50.0,
      )
      ...
      walker_load = st.number_input(
          "Moving Load Weight (kips)",
          min_value=0.01,
  ...
  if enable_udl:
    if "Pounds" in force_unit:
      w_mag_lb = st.number_input(
          "Intensity w (lbs/in)", min_value=1.0, value=500.0, step=50.0
      )
      w_magnitude = w_mag_lb / 1000.0
    else:
      w_magnitude = st.number_input(
          "Intensity w (kips/in)", min_value=0.001, value=0.5, step=0.1
      )

    col_u1, col_u2 = st.columns(2)
    with col_u1:
      x_start = st.number_input(
          "Start x1 (in)", min_value=0.0, max_value=float(L), value=0.0
      )
    with col_u2:
      x_end = st.number_input(
          "End x2 (in)", min_value=0.0, max_value=float(L), value=float(L)
      )
  else:
    w_magnitude = 0.0
  ...
  section_shape = st.selectbox(
      "Cross-Section Shape",
      ["Rectangular (Solid)", "Hollow Box / Tube", "I-Shape / Wide Flange"],
  )

  if section_shape == "Rectangular (Solid)":
    b = st.slider(
        "Width b (in)", min_value=1.0, max_value=24.0, value=8.0, step=0.5
    )
    h = st.slider(
        "Total Height h (in)", min_value=1.0, max_value=36.0, value=16.0, step=0.5
    )
    I = b * (h**3) / 12.0
    S = I / (h / 2.0)
    A_web = b * h
    ...
    b = st.slider(
        "Outer Width b (in)", min_value=2.0, max_value=24.0, value=8.0, step=0.5
    )
    h = st.slider(
        "Outer Height h (in)",
        min_value=2.0,
    ...
    t_wall = st.slider(
        "Wall Thickness t (in)", min_value=0.1, max_value=3.0, value=0.5, step=0.1
    )
    ...
    I = (t_web * (h_web**3) / 12.0) + 2 * (
        b * (t_flange**3) / 12.0
        + b * t_flange * ((h - t_flange) / 2.0) ** 2
    )
    S = I / (h / 2.0)
    A_web = t_web * h_web
  ...
  if section_shape == "Rectangular (Solid)":
    ...
    fig_2d.add_trace(
        go.Scatter(
            x=[-b_in / 2, b_in / 2, b_in / 2, -b_in / 2, -b_in / 2],
            y=[-h_in / 2, -h_in / 2, h_in / 2, h_in / 2, -h_in / 2],
            fill="toself",
            fillcolor="white",
  ...
  fig_2d.update_layout(
      xaxis=dict(
          title="Width b (in)", range=[-max(b, 4) * 0.7, max(b, 4) * 0.7]
      ),
      yaxis=dict(
          title="Height h (in)",
          range=[-max(h, 4) * 0.7, max(h, 4) * 0.7],
          scaleanchor="x",
          scaleratio=1,
      ),
  ...
  material_category = st.selectbox(
      "Material Category", ["Steel & Metals", "Wood & Timber", "Custom"]
  )
  ...
  factor_of_safety = st.number_input(
      "Factor of Safety (FOS)", min_value=0.1, value=1.5, step=0.1
  )
...
is_cantilever = ("Fixed" in support_A and "Free" in support_B) or (
    "Free" in support_A and "Fixed" in support_B
)
is_fixed_fixed = "Fixed" in support_A and "Fixed" in support_B
is_propped = (
    "Fixed" in support_A and ("Pinned" in support_B or "Roller" in support_B)
) or (("Pinned" in support_A or "Roller" in support_A) and "Fixed" in support_B)
...
if is_cantilever:
  if "Fixed" in support_A:
    ...
    RA = total_downward
    RB = 0.0
    sum_moments_fixed = sum(
        p * x_pos for p, x_pos in zip(all_P, all_x)
    ...
    RB = total_downward
    sum_moments_fixed = sum(
        p * (L - x_pos) for p, x_pos in zip(all_P, all_x)
    ) + (udl_total_force * (L - udl_center) if enable_udl else 0.0)
    MA_fix = 0.0
      ...
      if enable_udl and udl_length > 0:
        if xi > x_start:
          ...
          if eff_len > 0:
            ...
            m_sum += w_magnitude * eff_len * (
                xi - (x_start + eff_len / 2.0)
            )
      V[idx] = v_sum
      M[idx] = m_sum
  ...
  sum_m = sum(p * x_pos for p, x_pos in zip(all_P, all_x)) + (
      udl_total_force * udl_center if enable_udl else 0.0
  )
  MA_fix = sum_m / 2.0
  MB_fix = sum_m / 2.0
  ...
  sum_moments_A = sum(p * x_pos for p, x_pos in zip(all_P, all_x)) + (
      udl_total_force * udl_center if enable_udl else 0.0
  )
...
sigma_allow = (
    yield_strength / factor_of_safety if factor_of_safety > 0 else 1.0
)
...
m4.metric(
    ...
    delta=f"Limit: L/360 = {L/360:.2f} in",
)

st.divider()

# ================= 0. FREE BODY DIAGRAM (FBD) RIÊNG BIỆT (KHÔNG GỘP CHUNG) =================
...
# Gối A
if "Pinned" in support_A or "Hinged" in support_A:
  fig_fbd.add_trace(
      go.Scatter(
          x=[0],
          y=[-0.2],
          mode="markers",
          marker=dict(symbol="triangle-up", size=16, color="tab:blue"),
          showlegend=False,
      )
  )
elif "Roller" in support_A:
  fig_fbd.add_trace(
      go.Scatter(
          x=[0],
          y=[-0.15],
          mode="markers",
          marker=dict(symbol="circle", size=10, color="tab:blue"),
          showlegend=False,
      )
  )
elif "Fixed" in support_A:
  fig_fbd.add_trace(
      go.Scatter(
          x=[0, 0],
          y=[-0.4, 0.4],
          mode="lines",
          line=dict(color="tab:blue", width=6),
          showlegend=False,
      )
  )
...
fig_fbd.add_annotation(
    x=0,
    y=-0.5,
    text=f"R_A = {RA:.2f} kips",
    showarrow=True,
    arrowhead=2,
    arrowcolor="tab:red",
    font=dict(color="tab:red", size=11, family="Arial Black"),
)

# Gối B
if "Pinned" in support_B or "Hinged" in support_B:
  fig_fbd.add_trace(
      go.Scatter(
          x=[L],
          y=[-0.2],
          mode="markers",
          marker=dict(symbol="triangle-up", size=16, color="tab:orange"),
          showlegend=False,
      )
  )
elif "Roller" in support_B:
  fig_fbd.add_trace(
      go.Scatter(
          x=[L],
          y=[-0.15],
          mode="markers",
          marker=dict(symbol="circle", size=10, color="tab:orange"),
          showlegend=False,
      )
  )
elif "Fixed" in support_B:
  fig_fbd.add_trace(
      go.Scatter(
          x=[L, L],
          y=[-0.4, 0.4],
          mode="lines",
          line=dict(color="tab:orange", width=6),
          showlegend=False,
      )
  )
...
if enable_walker:
  fig_fbd.add_annotation(
      x=walker_pos,
      y=0.9,
      text=f"{icon_str} Moving ({walker_load} kips)",
      showarrow=True,
      arrowhead=2,
      arrowcolor="#2E7D32",
...
fig_fbd.update_layout(
    xaxis=dict(range=[-L * 0.05, L * 1.05], title="Beam Position x (in)"),
    yaxis=dict(range=[-1.0, 1.4], visible=False),
    height=250,
...
fig = make_subplots(
    ...
    subplot_titles=(
        ...
        f"2. Bending Moment Diagram (BMD) - Max: {max_m_kipft:.2f} kip-ft",
        f"3. Deflection Curve (Elastic Line) - Max: {max_deflection:.4f} in",
    ),
)
...
fig.update_layout(
    height=700, showlegend=False, hovermode="x unified", template="plotly_white"
)
fig.update_yaxes(title_text=v_unit_label, row=1, col=1)
fig.update_yaxes(title_text="Moment (kip-ft)", row=2, col=1)
fig.update_yaxes(title_text="Deflection (in)", row=3, col=1)
fig.update_xaxes(title_text="Beam Position x (in)", row=3, col=1)
...
with col_st3:
  st.write(
      f"- **Max Bending Stress ($\sigma_{{max}}$):** `{sigma_max:.2f} ksi`"
      f" (Allowable: `{sigma_allow:.2f} ksi`)"
  )
  st.write(
      f"- **Max Shear Stress ($\tau_{{max}}$):** `{tau_max:.3f} ksi`"
      f" (Allowable: `{tau_allow:.2f} ksi`)"
  )
  st.write(
      f"- **Max Deflection:** `{max_deflection:.4f} in` (Limit L/360:"
      f" `{L/360:.2f} in`)"
  )
