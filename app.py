...
output = Path("/mnt/data/simplified_beam_app.py")
...
STDOUT/STDERR
Created: /mnt/data/simplified_beam_app.py
Lines: 496


Mình đã update bản code ngắn gọn hơn dựa trên code cũ.

simplified_beam_app.py

Bản này giữ:

Simply supported beam: pinned–roller
Tối đa 5 point loads
Beam diagram và FBD riêng
Reaction forces
Shear, moment và deflection diagrams
Công thức tính toán
Material safety check
Deflection check L/360

Đã bỏ moving load, UDL, fixed support, wood, custom material và các cross-section phức tạp. 

simplified_beam_app.py
Code
File "/mount/src/interactive-beam-analysis/app.py", line 502, in <module>
    output.write_text(code, encoding="utf-8")
    ~~~~~~~~~~~~~~~~~^^^^^^^^^^^^^^^^^^^^^^^^
File "/usr/local/lib/python3.14/pathlib/__init__.py", line 809, in write_text
...
Bạn đã copy luôn phần code dùng để tạo file, nên Streamlit cố ghi vào /mnt/data/ và bị lỗi quyền truy cập.

Hãy thay toàn bộ app.py bằng file này:

app.py

Và thêm file này vào cùng repository:

requirements.txt

Trong app.py không được có các dòng như:
...
File mới đã được kiểm tra syntax và chỉ chứa code Streamlit chạy trực tiếp.
